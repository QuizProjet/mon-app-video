...
def gemini_generate_text(prompt, max_retries=2):
    """Gemini avec protection anti-429, sans cache de contenu : chaque génération est nouvelle."""
    now = time.time()
    ...
    if last and last.get("key") == prompt_key and now - last.get("time", 0) < 4:
        raise RuntimeError("Une génération identique vient d'être lancée. Attends quelques secondes puis relance si nécessaire.")
    last_error = None
    ...
    if _is_gemini_quota_error(last_error):
        raise RuntimeError("Gemini a atteint la limite de requêtes de ton offre actuelle. Attends un peu avant une nouvelle génération, ou vérifie les limites/billing de ton projet Google AI. Détail : " + str(last_error)) from last_error
    raise last_error
