...
# ============================================================
# QUIZVIDEO PRO — V4.2 DYNAMIC SHORTS ENGINE
# ============================================================
...
def gemini_generate_text(prompt, max_retries=2):
    ...
    last = st.session_state.get("_gemini_last_request")
    if last and last.get("key") == prompt_key and now - last.get("time", 0) < 4:
        raise RuntimeError("Une génération identique vient d'être lancée. Attends quelques secondes puis relance.")
    last_error = None
    for attempt in range(max_retries + 1):
        try:
            ...
            if not text: raise ValueError("Gemini a renvoyé une réponse vide.")
            st.session_state._gemini_last_request = {"key": prompt_key, "time": time.time()}
            return text, False
        except Exception as e:
            last_error = e
            if not _is_gemini_quota_error(e) or attempt >= max_retries: break
    ...
    if _is_gemini_quota_error(last_error):
        raise RuntimeError("Gemini a atteint la limite de requêtes. Attends un peu avant de réessayer.") from last_error
    raise last_error
...
def parse_quiz_csv(uploaded_file):
    ...
    for row in reader:
        ...
        ans=clean_text(row.get(fields["reponse_correcte"],"A"))
        exp=clean_text(row.get(fields["explication"],"")) if "explication" in fields else ""
        if q and all(opts): data.append({"question":q,"options":opts,"reponse_correcte":ans,"explication":exp})
...
with tab1:
    ...
    if mode_q=="🤖 IA Gemini":
        st.caption("Le style visuel ne change pas le contenu. Pour changer les questions, modifie le « Sujet du quiz ». Chaque clic produit un nouveau lot.")
...
ID de génération : {nonce}. Retourne UNIQUEMENT un JSON valide sous forme de tableau."""
    ...
    if st.session_state.get("q_data"):
        ...
        if st.button("🎬 Générer le Short Quiz V4.2",key="makeq"):
            try:
                with st.spinner("Création du Short dynamique V4.2..."):
                    with tempfile.TemporaryDirectory() as tmp:
                        ...
                        for idx,q in enumerate(st.session_state.q_data):
                            ...
                            exp_text=f"La bonne réponse est {q['options'][corr]}. {q.get('explication','') or 'Bravo !'}"
                        ...
                        st.success("✅ Short Quiz V4.2 terminé.")
                        st.video(data)
...
with tab2:
    ...
    st.markdown('<div class="qvp-card"><b>🗣️ Studio Vocabulaire</b><div class="qvp-small">Mot Français → Chrono → Traduction bilingue</div></div>', unsafe_allow_html=True)
    hook_v=st.text_input("Hook","Tu prononces mal ces 10 mots !",key="hv")
    ...
    if st.button("✨ Générer le vocabulaire par IA",key="genv",use_container_width=True):
        if not api_key: st.error("Ajoute ta clé API Gemini dans la barre latérale.")
        else:
            try:
                nonce=random.randint(100000,999999999)
                prompt=f'''Génère exactement {nb_v} mots français DIFFERENTS avec leur traduction en {langue_v} sur le sujet « {th_v} ». ID de génération : {nonce}. Retourne UNIQUEMENT un JSON valide: [{{"fr":"...","trad":"..."}}]'''
                res_text, _ = gemini_generate_text(prompt)
                st.session_state.v_data=parse_json(res_text)[:nb_v]
                st.success(f"✅ {len(st.session_state.v_data)} mots de vocabulaire prêts.")
            except Exception as e: st.error(f"Erreur Gemini : {e}")
    if st.session_state.get("v_data"):
        if st.button("🎬 Générer la vidéo Vocabulaire V4.2",key="makev"):
            try:
                with st.spinner("Création du Short vocabulaire V4.2..."):
                    with tempfile.TemporaryDirectory() as tmp:
                        ...
                        with open(final,"rb") as f: data=f.read()
                        st.success("✅ Short Vocabulaire V4.2 terminé.")
                        st.video(data)
