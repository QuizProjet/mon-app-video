...
def _layout(module="quiz"):
    ...
    defaults={
        ...
        "style2_question_y":1430,"style2_list_x":42,"style2_list_y":180,"style2_list_size":28,
    }
    if module=="vocab":
...
def _style2_country_code(question):
    """Petit repère visuel pour les questions de capitales courantes."""
    q=clean_text(question).lower()
    mapping={
        "australie":"AU","canada":"CA","espagne":"ES","italie":"IT","turquie":"TR",
        "colombie":"CO","thaïlande":"TH","thailande":"TH","france":"FR","allemagne":"DE",
        "japon":"JP","chine":"CN","brésil":"BR","bresil":"BR","mexique":"MX","portugal":"PT",
        "royaume-uni":"GB","angleterre":"GB","états-unis":"US","etats-unis":"US","usa":"US",
        "inde":"IN","maroc":"MA","tunisie":"TN","algérie":"DZ","algerie":"DZ",
        "égypte":"EG","egypte":"EG","grèce":"GR","grece":"GR","suisse":"CH","belgique":"BE",
    }
    for name,code in mapping.items():
        if name in q: return code
    return ""

def _draw_simple_flag(draw, code, cx, cy, w=86, h=52):
    """Drapeaux simplifiés, sans dépendre d'une police emoji."""
    x0,y0=cx-w//2,cy-h//2
    draw.rounded_rectangle((x0,y0,x0+w,y0+h),radius=8,fill=(245,245,245),outline=(255,255,255),width=2)
    if code in ("FR","IT","BE"):
        cols={"FR":((38,85,164),(245,245,245),(239,65,68)),"IT":((35,145,80),(245,245,245),(225,55,55)),"BE":((25,25,25),(248,210,45),(225,45,45))}[code]
        sw=w//3
        for i,c in enumerate(cols): draw.rectangle((x0+i*sw,y0,x0+(i+1)*sw,y0+h),fill=c)
    elif code in ("ES","DE","CO","TH"):
        patterns={
            "ES":((210,45,45),(245,205,55),(210,45,45)),
            "DE":((25,25,25),(215,45,45),(245,195,55)),
            "CO":((245,205,35),(245,205,35),(45,90,175)),
            "TH":((190,35,50),(245,245,245),(35,45,100)),
        }
        a,b,c=patterns[code]
        if code=="TH":
            draw.rectangle((x0,y0,x0+w,y0+h),fill=a); draw.rectangle((x0,y0+9,x0+w,y0+h-9),fill=b); draw.rectangle((x0,y0+17,x0+w,y0+h-17),fill=c)
        elif code=="CO":
            draw.rectangle((x0,y0,x0+w,y0+h//2),fill=a); draw.rectangle((x0,y0+h//2,x0+w,y0+3*h//4),fill=b); draw.rectangle((x0,y0+3*h//4,x0+w,y0+h),fill=c)
        else:
            hh=h//3
            for i,col in enumerate((a,b,c)): draw.rectangle((x0,y0+i*hh,x0+w,y0+(i+1)*hh),fill=col)
    elif code in ("CA","US","GB","AU","NZ"):
        draw.rectangle((x0,y0,x0+w,y0+h),fill=(215,35,45) if code in ("CA","AU") else (245,245,245))
        if code=="CA":
            draw.rectangle((x0+w//4,y0,x0+3*w//4,y0+h),fill=(245,245,245));
            draw.ellipse((cx-8,cy-15,cx+8,cy+15),fill=(215,35,45))
        elif code=="US":
            for i in range(7): draw.rectangle((x0,y0+i*h//7,x0+w,y0+(i+1)*h//7),fill=(210,45,55) if i%2==0 else (245,245,245))
            draw.rectangle((x0,y0,x0+w//2,y0+h//2),fill=(35,65,145))
        elif code=="GB":
            draw.rectangle((x0,y0,x0+w,y0+h),fill=(35,65,145)); draw.line((x0,y0,x0+w,y0+h),fill="white",width=10); draw.line((x0+w,y0,x0,y0+h),fill="white",width=10); draw.line((cx,y0,cx,y0+h),fill=(210,45,55),width=6); draw.line((x0,cy,x0+w,cy),fill=(210,45,55),width=6)
        else:
            draw.rectangle((x0,y0,x0+w,y0+h),fill=(35,65,145)); draw.ellipse((cx-10,cy-10,cx+10,cy+10),fill=(245,245,245)); draw.ellipse((cx-5,cy-5,cx+5,cy+5),fill=(215,45,55))
    elif code=="TR":
        draw.rectangle((x0,y0,x0+w,y0+h),fill=(220,40,50)); draw.ellipse((cx-16,cy-14,cx+12,cy+14),fill="white"); draw.ellipse((cx-10,cy-12,cx+18,cy+12),fill=(220,40,50)); draw.polygon([(cx+6,cy),(cx+23,cy-8),(cx+18,cy),(cx+23,cy+8)],fill="white")
    elif code=="JP":
        draw.rectangle((x0,y0,x0+w,y0+h),fill="white"); draw.ellipse((cx-13,cy-13,cx+13,cy+13),fill=(210,45,55))
    elif code=="CN":
        draw.rectangle((x0,y0,x0+w,y0+h),fill=(220,40,50)); draw.ellipse((x0+13,y0+10,x0+25,y0+22),fill=(250,215,45))
    else:
        draw.rectangle((x0,y0,x0+w,y0+h),fill=(235,235,235))

def draw_style2_frame(items, active_idx, theme_name, channel, bg_file=None, timer=None, timer_fraction=1.0, answer_reveal=False, motion=0.0, video_title="Culture Générale", blur_background=False, show_question_number=False):
    """Style 2 basé sur la vidéo de référence fournie par l'utilisateur.

    Une seule page 9:16 reste à l'écran : titre en haut, historique des réponses
    à gauche, question active dans une carte basse, visuel/drapeau sous la carte
    et minuteur au centre pendant la réflexion. La question précédente disparaît
    quand la suivante arrive; seules les réponses révélées restent dans la liste.
    """
    ...
    if alpha:
        base=Image.alpha_composite(base.convert("RGBA"),Image.new("RGBA",(WIDTH,HEIGHT),(0,0,0,alpha))).convert("RGB")
    img=add_top_glow(base,theme,1.0+0.08*math.sin(float(motion)*math.pi*2)); draw=ImageDraw.Draw(img)
    total=max(1,len(items)); active_idx=max(0,min(int(active_idx),total-1)); active=items[active_idx]

    # En-tête noir compact comme la vidéo de référence.
    title=clean_text(video_title or "GRAND QUIZ")
    title_size=int(cfg.get("title_size",46)); tf=get_font(title_size)
    title_y=int(cfg.get("title_y",42))
    if cfg.get("show_title",True):
        tw=text_width(draw,title,tf)
        box_w=min(940,tw+70); left=(WIDTH-box_w)//2
        draw.rounded_rectangle((left,title_y-12,left+box_w,title_y+title_size+18),radius=20,fill=(12,8,8),outline=_hex_rgb(cfg.get("primary"),theme["accent"]),width=2)
        draw.text(((WIDTH-tw)//2,title_y),title,font=tf,fill=_hex_rgb(cfg.get("text"),(255,255,255)))

    # Historique fixe à gauche : uniquement les réponses déjà gagnées.
    list_x=int(cfg.get("style2_list_x",42)); list_y=int(cfg.get("style2_list_y",180)); line_h=53; lf=get_font(int(cfg.get("style2_list_size",28)))
    for n in range(total):
        yy=list_y+n*line_h
        draw.text((list_x,yy),f"{n+1}.",font=lf,fill=(245,245,245))
        if n < active_idx or (n==active_idx and answer_reveal):
            opts=items[n].get("options",[]); rc=clean_text(items[n].get("reponse_correcte","A")).upper()[:1]
            try: ans=clean_text(opts["ABCD".index(rc)])
            except Exception: ans=""
            draw.text((list_x+40,yy),ans,font=lf,fill=(245,245,245))

    # Numéro de la question actuelle, comme la transition N°3 de la référence.
    if show_question_number:
        nf=get_font(54); nt=f"N°{active_idx+1}"; nw=text_width(draw,nt,nf)
        draw.text(((WIDTH-nw)//2,1210),nt,font=nf,fill=(255,255,255))

    # Minuteur central pendant la réflexion.
    if timer is not None and cfg.get("show_timer",True):
        draw_inline_timer(draw,theme,int(cfg.get("timer_x",540)),int(cfg.get("timer_y",900)),timer,timer_fraction,"quiz")

    # Question active dans la zone basse : elle remplace toujours la précédente.
    q=clean_text(active.get("question","")); qf=get_font(max(32,int(cfg.get("question_size",47)*1.05)))
    qlines=wrap_text(q,qf,820)[:3]
    box_y=int(cfg.get("style2_question_y",1430)); box_h=max(150,55*len(qlines)+90); box_w=900; left=(WIDTH-box_w)//2; right=left+box_w
    draw.rounded_rectangle((left,box_y,right,box_y+box_h),radius=22,fill=(15,12,12),outline=(255,255,255),width=2)
    yy=box_y+30
    for line in qlines:
        tw=text_width(draw,line,qf); draw.text(((WIDTH-tw)//2,yy),line,font=qf,fill="white"); yy+=int(qf.size*1.08)

    # Petit visuel sous la question (drapeau pour les capitales courantes).
    code=_style2_country_code(q); _draw_simple_flag(draw,code,540,box_y+box_h+55,w=82,h=50) if code else None

    # Révélation au centre-bas : la réponse apparaît en vert, puis rejoint la liste.
    if answer_reveal:
        opts=active.get("options",[]); rc=clean_text(active.get("reponse_correcte","A")).upper()[:1]
        try: ans=clean_text(opts["ABCD".index(rc)])
        except Exception: ans=""
        rf=get_font(68); text="✓ "+ans; rw=text_width(draw,text,rf)
        pill_y=box_y-105
        draw.rounded_rectangle(((WIDTH-rw)//2-28,pill_y,(WIDTH+rw)//2+28,pill_y+86),radius=28,fill=_hex_rgb(cfg.get("correct"),theme["success"]))
        draw.text(((WIDTH-rw)//2,pill_y+7),text,font=rf,fill="white")
...
def render_layout_editor(module, key_prefix):
    ...
    defaults={
        ...
        "bg_mode":"✨ Automatique",
        "style2_question_y":1430,
        "style2_list_x":42,"style2_list_y":180,"style2_list_size":28,
    }
    ...
    with tabs[1]:
        ...
        with c1:
            ...
            if is_quiz:
                st.markdown("**Style 2 — liste / question active**")
                st.slider("Liste X",20,140,st.session_state.get(p+"style2_list_x",42),key=p+"style2_list_x")
                st.slider("Liste Y",120,360,st.session_state.get(p+"style2_list_y",180),key=p+"style2_list_y")
                st.slider("Taille de la liste",20,40,st.session_state.get(p+"style2_list_size",28),key=p+"style2_list_size")
                st.slider("Question active Y",1250,1580,st.session_state.get(p+"style2_question_y",1430),key=p+"style2_question_y")
        ...
        try:
            ...
            if style_q_full.startswith("Style 2"):
                ...
                if preview_state_q=="Q1 + minuteur":
                    preview=draw_style2_frame(demo,0,theme_q,channel_q,sample_bg,timer=3,timer_fraction=.72,video_title=th_q,blur_background=True,show_question_number=False)
                elif preview_state_q=="Q2 + R1":
                    preview=draw_style2_frame(demo,1,theme_q,channel_q,sample_bg,timer=2,timer_fraction=.5,video_title=th_q,blur_background=True,show_question_number=False)
                else:
                    preview=draw_style2_frame(demo,2,theme_q,channel_q,sample_bg,answer_reveal=True,video_title=th_q,show_question_number=True)
                            ...
                            if style_q_full.startswith("Style 2"):
                                ...
                                for idx,q in enumerate(items):
                                    ...
                                    for j in range(q_steps):
                                        t=j/max(1,q_steps-1)
                                        frames.append((draw_style2_frame(items,idx,theme_q,channel_q,bg_question,answer_reveal=False,motion=t*.7,video_title=th_q,blur_background=False,show_question_number=True),qdur/q_steps))
                                    ...
                                    for j in range(cd_steps):
                                        ...
                                        frames.append((draw_style2_frame(items,idx,theme_q,channel_q,bg_question,timer,timer_frac,False,t,th_q,blur_background=True,show_question_number=False),cdur/cd_steps))
                                    a_steps=max(8,int(adur*12))
                                    for j in range(a_steps):
                                        t=j/max(1,a_steps-1)
                                        frames.append((draw_style2_frame(items,idx,theme_q,channel_q,bg_question,answer_reveal=True,motion=1.0+t*.5,video_title=th_q,blur_background=False,show_question_number=True),adur/a_steps))
