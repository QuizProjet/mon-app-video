...
st.header("1. Générateur Quizz Shorts Pro (Format Court & Dynamique)")
...
outro_q_custom = st.text_input("Phrase d'Outro / CTA Final", "Quel est ton score ? Dis-le en commentaire !", key="oq")
...
if 'q_data' in st.session_state and st.session_state['q_data']:
    if st.button("🎬 Générer la vidéo Shorts Rapide & Dynamique MP4"):
        with st.spinner("Montage accéléré style TikTok..."):
            try:
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    # 2. Questions Loop (Format court)
                    for idx, q in enumerate(st.session_state['q_data']):
                        # Énoncé Question (toutes les options sont affichées d'un coup)
                    ...
                    st.success("✅ Vidéo TikTok / Shorts générée avec succès !")
                    
                    with open(out_mp4, "rb") as f:
                        video_bytes = f.read()
                    st.video(video_bytes)
                    st.download_button("📥 Télécharger le MP4 Shorts Pro", data=video_bytes, file_name="quizz_viral_pro.mp4", mime="video/mp4")
            except Exception as e:
