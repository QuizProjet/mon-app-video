...
# ============================================================
st.set_page_config(
    page_title="Studio TikTok & Shorts Pro V3.2",
    page_icon="🎬",
    layout="wide"
)
st.title("🎬 Studio TikTok & Shorts Pro V3.2")
st.caption("Quiz + Vocabulaire • Synchronisation Visuelle & Audio Parfaite")
...
# ============================================================
# SYNTHESE AUDIO ET OUTILS FFMPEG
# ============================================================
...
def get_correct_index(rep_str, options):
    ...
    # Si la réponse commence par A, B, C ou D
    for i, letter in enumerate(["A", "B", "C", "D"]):
        if clean_rep.startswith(letter):
            return i
    # Sinon, on cherche si le texte correspond à une des options
            ...
            with st.spinner("Montage en cours..."):
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    for idx, q in enumerate(st.session_state['q_data']):
                        ...
                        # C. Révélation Visuelle (Bonne option en vert) + Son Ding
                        corr_idx = get_correct_index(q['reponse_correcte'], q['options'])
                        ...
                        # D. Explication audio maintenue sur la frame de révélation
                    ...
                    st.success("✅ Vidéo V3.2 parfaitement synchronisée !")
                    with open(final, "rb") as f:
                        video_bytes = f.read()
                        st.video(video_bytes)
                        st.download_button("⬇️ Télécharger la vidéo Quiz V3.2", data=video_bytes, file_name="quizvideo_pro_v3_2.mp4", mime="video/mp4")
        ...
        if st.button("🎬 Générer le MP4 Vocabulaire"):
            with st.spinner("Montage du vocabulaire..."):
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    concatenate_clips(clips, final, tmpdir)
                    st.success("✅ Vidéo Vocabulaire V3.2 prête !")
                    with open(final, "rb") as f:
                        video_bytes = f.read()
                        st.video(video_bytes)
                        st.download_button("⬇️ Télécharger la vidéo Vocabulaire", data=video_bytes, file_name="vocabulaire_pro_v3_2.mp4", mime="video/mp4")
